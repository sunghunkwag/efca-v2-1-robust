...
class ProbeNetwork(nn.Module):
    ...
    def probe_h_jepa(self, features: torch.Tensor, loss: Optional[torch.Tensor] = None) -> torch.Tensor:
        ...
        Args:
            features (torch.Tensor): Online features from H-JEPA
                - (B, C, H, W) for vision
                - (B, D) for state
        ...
        if len(features.shape) == 4:
            # Spatial pooling to get global features (Vision)
            B, C, H, W = features.shape
            pooled = features.mean(dim=[2, 3])  # (B, C)
        elif len(features.shape) == 2:
            # State features are already global (B, D)
            pooled = features
        else:
            raise ValueError(f"Unsupported feature shape for probe: {features.shape}")
